...
# (assets/ask/app.js), plus the shared submit client every ask page loads
# alongside it (assets/shared/submit.js).
#
...
# suite would notice a syntax error or a renamed global in it — and an ask page
# whose Submit throws still looks fine until the user clicks.
...
# sides at once still breaks every already-authored ask page, so it must fail here.
run_node_test "shared submit.js defines the globals ask/app.js calls" "
