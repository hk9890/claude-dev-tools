...
assert_eq "valid: total files"         "2"    "$(json_val "$OUT" "d['totals']['files']")"
assert_eq "valid: total lines_covered" "4"    "$(json_val "$OUT" "d['totals']['lines_covered']")"
assert_eq "valid: total lines_total"   "10"   "$(json_val "$OUT" "d['totals']['lines_total']")"
assert_eq "valid: total pct"           "40.0" "$(json_val "$OUT" "d['totals']['pct']")"
# a.py: covered {1,2,3,5}=4, uncovered {4,6,7}=3 → 4/7 = 57.1%
assert_eq "valid: a.py covered"        "4"          "$(json_val "$OUT" "[f for f in d['files'] if f['path']=='src/a.py'][0]['lines_covered']")"
assert_eq "valid: a.py pct"            "57.1"       "$(json_val "$OUT" "[f for f in d['files'] if f['path']=='src/a.py'][0]['pct']")"
assert_eq "valid: a.py covered_ranges" "[[1, 3], [5, 5]]" "$(json_val "$OUT" "[f for f in d['files'] if f['path']=='src/a.py'][0]['covered_ranges']")"
assert_eq "valid: b.py pct"            "0.0"        "$(json_val "$OUT" "[f for f in d['files'] if f['path']=='src/b.py'][0]['pct']")"

# stdin is equivalent to a file argument
OUT=$("$SCRIPT" < "$DIR/ok.json")
assert_eq "valid via stdin: total files" "2" "$(json_val "$OUT" "d['totals']['files']")"
...
assert_eq "overlap: lines_total"       "8"        "$(json_val "$OUT" "d['files'][0]['lines_total']")"
assert_eq "overlap: lines_covered"     "5"        "$(json_val "$OUT" "d['files'][0]['lines_covered']")"
assert_eq "overlap: uncovered_ranges"  "[[6, 8]]" "$(json_val "$OUT" "d['files'][0]['uncovered_ranges']")"
...
assert_eq "merge: covered_ranges" "[[1, 4]]" "$(json_val "$OUT" "d['files'][0]['covered_ranges']")"
...
assert_eq "path: normalized" "pkg/mod.py" "$(json_val "$OUT" "d['files'][0]['path']")"
