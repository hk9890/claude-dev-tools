...
6. **Recommended next flow** — `docs-update` for factual fixes, `docs-improve` for structural fixes, or `none` if `clean`.
...
3. The report includes all required sections (Headline, Scope, Coverage table, Findings, Not checked, Recommended next flow).
