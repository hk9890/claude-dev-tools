...
When non-standard docs exist, the auditor flags consolidation candidates (merge/split/slim) as suggestions — it does not decide or apply keep/merge/split/delete.

- Prefer canonical steering docs as operating layer.
- Keep non-standard files only with explicit scoped justification.
- Flag stale routes left after merges/deletions.
