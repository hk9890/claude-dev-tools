...
## 5. Schema-agnostic server — each mode owns its payload
...
## 6. Per-mode browser assets under `assets/<mode>/`
...
One clarification for the feedback-mode Apply loop (see rule 12): an Apply loop
...
## 9. (ask mode) Partial feedback is always accepted — verdict is optional
...
## 10. (ask mode) Every choice-style question carries an always-visible note field
...
## 11. (feedback mode) Block-anchored comments — no fragile text offsets
...
## 12. (feedback mode) The Apply loop — iterate without breaking the one-shot server
