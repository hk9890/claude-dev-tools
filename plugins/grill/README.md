...
The simplicity half overlaps deliberately with `project-quality`'s complexity-review
principles — both descend from the same lean-systems ideas. They are kept as separate,
self-contained copies because each plugin stands alone; grill's set adds the risk lenses
a pre-commitment stress-test needs and that a complexity review does not.
