...
| [`html-visualization`](plugins/html-visualization/README.md) | Interactive HTML the user opens in a browser, then sends back to Claude — the `visualize-html` skill renders question forms (ask mode), content for inline commenting (feedback mode), or display-only visualizations (visualize mode); a shared one-shot Node server captures the response and re-invokes Claude |
