...
| [`html-visualization`](plugins/html-visualization/README.md) | Interactive HTML the user opens in a browser, then sends back to Claude — `html-ask` renders question and decision forms, `html-feedback` renders content for inline commenting; a shared one-shot Node server captures the response and re-invokes Claude |
