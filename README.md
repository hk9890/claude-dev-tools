...
| [`project-review`](plugins/project-review/) | Read-only adversarial project reviews — codebase (consistency, structure, architecture), tests, and docs — each a standalone lens that challenges the artifact against the project's own standards and returns a prioritized action list; the codebase lens also writes a standalone Markdown report with Mermaid diagrams and numbered deepening proposals |
...
| [`project-auto-work`](plugins/project-auto-work/) | Unattended adversarial audits that work the project on their own and report back — includes `test-tests`: an empirical, language-independent test-suite strength audit that injects mutants, no-op edits, and delays to prove whether the tests actually detect bugs; reports findings and proposals, never keeps an edit |
| [`instruction-writing`](plugins/instruction-writing/) | Guidance for writing the artifacts an agent harness reads — project docs, skills, agents, workflow prompts — so they stay token-efficient and predictable; includes `writing-skills`: the vocabulary and principles for authoring and editing Claude Code skills |
